...
@customElement('google-home-detail-slider')
export class GoogleHomeDetailSlider extends LitElement {
    @property() protected min: number;
    @property() protected max: number;
    @property() protected step: number;
    @property() protected startAngle: number;
    @property() protected arcLength: number;
    @property() protected handleSize: number;
    @property() protected handleZoom: number;
    @property() protected disabled: boolean;
    ...
    @property() private low?: number;
    @property() private _rotation?: any;
    @property() private _reverseOrder?: boolean;
    ...
    _angleInside(angle) {
        ...
        let a =
            ((this.startAngle + this.arcLength / 2 - angle + 180 + 360) % 360) -
            180;
    ...
    _angle2value(angle) {
        return (
            Math.round(
                ((angle / this._len) * (this.max - this.min) + this.min) /
                    this.step
            ) * this.step
        );
    }
    ...
    drag(ev) {
        ...
        const x =
            mouseX -
            (rect!.left + (boundaries.left * rect!.width) / boundaries.width);
        const y =
            mouseY -
            (rect!.top + (boundaries.up * rect!.height) / boundaries.height);
    ...
    updated(changedProperties) {
        ...
        if (
            ...
            // @ts-ignore
            this.shadowRoot?.querySelector('svg')?.style.vectorEffect !==
                undefined
        )
            return;
            ...
            >
              ...
              <path
                ...
                d=${this._renderArc(
                    ...
                    this._value2angle(
                        this.high != null ? this.high : this.value
                    )
                )}
              />`
                        : ``}
